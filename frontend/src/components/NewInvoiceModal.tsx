...
import { X, Loader2, FileText, Calendar, IndianRupee, Briefcase, Plus, CheckCircle2, Share2, Download, ExternalLink, Zap, Receipt, ShieldCheck, Target } from "lucide-react";
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 border border-slate-100">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[11px] font-semibold text-slate-700 uppercase tracking-tight">Apply GST (18%)</span>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                <div className="p-5 rounded-xl bg-slate-900">
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                            <span>Subtotal</span>
                                            <span className="font-mono text-slate-200">₹{Number(formData.amount || 0).toLocaleString()}</span>
                                        </div>
                                        {formData.includeGst && (
                                            <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                                                <span>Tax (18%)</span>
                                                <span className="font-mono text-blue-400">+₹{Math.round(Number(formData.amount || 0) * 0.18).toLocaleString()}</span>
                                            </div>
                                        )}
                                        <div className="pt-2 border-t border-white/10 flex justify-between items-center">
                                            <span className="text-xs font-bold text-white uppercase tracking-wider">Total</span>
                                            <span className="text-lg font-bold text-blue-400 font-mono">
                                                ₹{Math.round(Number(formData.amount || 0) * (formData.includeGst ? 1.18 : 1)).toLocaleString()}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
                                ...
                                <div className="flex justify-between text-xs font-bold pt-2 border-t border-slate-200/50">
                                    ...
                                    <span className="text-teal-600 font-mono">₹{createdInvoice.total.toLocaleString()}</span>
                                </div>
                            </div>
