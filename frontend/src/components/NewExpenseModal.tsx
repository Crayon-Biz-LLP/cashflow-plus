...
import { X, Loader2, Receipt, Calendar, IndianRupee, DollarSign, Briefcase, FileText, Plus, Landmark, Tag, Target, Zap, ShieldCheck, Percent } from "lucide-react";
...
export default function NewExpenseModal({ isOpen, onClose, onSuccess }: NewExpenseModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showSuccessToast(`${formData.category} expense of ${currencySymbol}${Number(formData.amount).toLocaleString()} logged.`);
            onSuccess();
            onClose();
        } catch (error: any) {
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    <div className="space-y-4">
                            ...
                            >
                                ...
                                {bankAccounts.map(b => (
                                    <option key={b.bankAccountId} value={b.bankAccountId}>
                                        {b.bankName} (••{b.accountNumber.slice(-4)}) — {currencySymbol}{b.currentBalance.toLocaleString()}
                                    </option>
                                ))}
                            </select>
                        </div>
                        ...
                        {/* Tax Profile Link */}
                        <div className="space-y-1.5">
                            <label className="text-[11px] font-semibold text-slate-600 ml-0.5 uppercase tracking-wider">Applicable Tax</label>
                            <div className="relative">
                                <ShieldCheck size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
                                <select
                                    className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-medium appearance-none cursor-pointer"
