            ...
            <div className="px-4 md:px-8 pb-12">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    ...
                    <div className="flex items-center gap-2 md:gap-3">
                        <button className="p-2.5 rounded-xl border border-gray-200 bg-white text-gray-400 hover:text-[#ee2a7b] hover:bg-rose-50 hover:border-rose-100 transition-all shadow-sm relative group animate-in slide-in-from-right-2 duration-500 delay-100">
                            <Bell size={18} className="group-hover:rotate-[15deg] transition-transform" />
                            <span className="absolute top-2.5 right-2.5 w-2 h-2 rounded-full bg-[#ee2a7b] border-2 border-white animate-pulse" />
                        </button>
                ...
                <div className="grid grid-cols-12 gap-6">
                    ...
                    {/* Secondary Column / Notification Bar Sidebar */}
                    <div className="col-span-12 lg:col-span-4 space-y-6 lg:sticky lg:top-24 h-fit">
                        {/* Priorities/Alerts */}
                        ...
                        <div className="p-6 rounded-[2rem] bg-white border border-gray-100 shadow-premium">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button className="p-1.5 rounded-lg text-gray-400 hover:text-teal-600 hover:bg-teal-50 transition-all">
                                    <Bell size={14} />
                                </button>
                            </div>
                            <div className="space-y-6">
                                {(stats?.activity || []).map((item: any, i: number) => {
                                    ...
                                    return (
                                        <div key={i} className="relative pl-7 before:absolute before:left-1.5 before:top-2 before:bottom-[-30px] before:w-[1px] before:bg-gray-100 last:before:hidden">
                                            <div className="absolute left-0 top-1 w-3.5 h-3.5 rounded-full border-[3px] border-white shadow-sm ring-1 ring-gray-100" style={{ background: item.color }} />
                                ...
                                {(stats?.timelineAlerts || []).length > 0 && (
                                    <div className="pt-6 border-t border-gray-50 mt-2 animate-in fade-in slide-in-from-bottom-2 duration-700">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                            <Bell size={10} /> System Notifications
                                        </p>
                                        <div className="space-y-4">
                                            {stats.timelineAlerts.map((alert: any, idx: number) => (
                                                <div key={idx} className="flex gap-3 items-start group hover:translate-x-1 transition-transform">
                                                    <div className={`p-1.5 rounded-lg shrink-0 ${alert.priority === 'critical' ? 'bg-rose-50 text-[#ee2a7b]' : 'bg-amber-50 text-amber-500'}`}>
                                                        {alert.priority === 'critical' ? <AlertTriangle size={14} /> : <Bell size={14} />}
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="text-[11px] font-black text-gray-900 leading-tight underline decoration-gray-100 underline-offset-2">{alert.title}</p>
                                                        <p className="text-[10px] font-medium text-gray-500 leading-relaxed mt-1 line-clamp-2">{alert.message}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
