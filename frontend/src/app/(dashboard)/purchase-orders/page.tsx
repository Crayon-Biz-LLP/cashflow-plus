...
const statusColors: Record<string, { color: string; bg: string }> = {
    ...
    "Partially Received": { color: "#eab308", bg: "#fefce8" },
    Received: { color: "#16a34a", bg: "#f0fdf4" },
    Billed: { color: "#7c3aed", bg: "#f5f3ff" },
...
export default function PurchaseOrdersPage() {
    ...
    const [form, setForm] = useState({
        vendorId: "", isInterState: false, expectedDelivery: "", notes: "",
    ...
    async function loadOrders() {
        try {
            const data = await api.getPurchaseOrders(statusFilter === "All" ? undefined : statusFilter);
            setOrders(data);
        ...
        try {
            await api.createPurchaseOrder({
                ...
                items: form.items.map(it => ({
                    name: it.name, hsnCode: it.hsnCode,
                    quantity: Number(it.quantity), rate: Number(it.rate), gstRate: Number(it.gstRate),
                })),
            });
            setShowModal(false);
            setForm({ vendorId: "", isInterState: false, expectedDelivery: "", notes: "", items: [{ name: "", hsnCode: "", quantity: 1, rate: 0, gstRate: 18 }] });
            loadOrders();
    ...
    const filtered = orders.filter(o => !search || (o.vendorName || "").toLowerCase().includes(search.toLowerCase()) || o.poId.toLowerCase().includes(search.toLowerCase()));

    return (
        <div>
            <Topbar title="Purchase Orders" subtitle="Vendor orders with auto-billing & inventory stock updates on receipt" />
            <div className="px-8 pb-8">
                <div className="grid grid-cols-4 gap-4 mb-6">
                    {Object.entries({ All: orders.length, Confirmed: orders.filter(o => o.status === "Confirmed").length, Received: orders.filter(o => o.status === "Received").length, Billed: orders.filter(o => o.status === "Billed").length }).map(([label, count]) => (
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <input className="pl-9 pr-3 py-2 text-xs rounded-xl border" style={{ borderColor: "var(--card-border)", width: 240 }}
                                placeholder="Search by vendor or PO ID..." value={search} onChange={e => setSearch(e.target.value)} />
                        </div>
                    </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr style={{ background: "var(--black-50)" }}>
                                <th className="text-left text-xs font-semibold px-5 py-3" style={{ color: "var(--black-500)" }}>PO #</th>
                                <th className="text-left text-xs font-semibold px-5 py-3" style={{ color: "var(--black-500)" }}>Vendor</th>
                        ...
                        <tbody>
                            {filtered.map(o => {
                                ...
                                return (
                                    <tr key={o.poId} className="border-t hover:bg-gray-50" style={{ borderColor: "var(--black-100)" }}>
                                        <td className="px-5 py-3">
                                            <span className="font-mono text-xs font-bold" style={{ color: "var(--teal-600)" }}>{o.poId}</span>
                                            <p className="text-xs" style={{ color: "var(--black-400)" }}>
                                                {new Date(o.orderDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                                            </p>
                                        </td>
                                        <td className="px-5 py-3">
                                            <p className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>{o.vendorName || o.vendorId}</p>
                                        </td>
                                        <td className="px-5 py-3 text-xs" style={{ color: "var(--black-500)" }}>{o.items.length} items</td>
                                        ...
                                        <td className="px-5 py-3 text-right">
                                            ...
                                            {o.isInterState ? (
                                                ...
                                                <p className="text-xs" style={{ color: "var(--black-400)" }}>C: {formatINR(o.cgst)} S: {formatINR(o.sgst)}</p>
                                            )}
                                        </td>
                                        ...
                                        <td className="px-5 py-3 text-center">
                                            {o.status === "Draft" && (
                                                <button className="text-xs font-semibold px-3 py-1 rounded-lg" style={{ background: "#eff6ff", color: "#2563eb" }}
                                                    onClick={() => handleStatusChange(o.poId, "Confirmed")}>Confirm</button>
                                            )}
                                            {o.status === "Confirmed" && (
                                                <button className="text-xs font-semibold px-3 py-1 rounded-lg" style={{ background: "#f0fdf4", color: "#16a34a" }}
                                                    onClick={() => handleStatusChange(o.poId, "Received")}>Receive</button>
                                            )}
                                            {o.status === "Received" && (
                                                <button className="text-xs font-semibold px-3 py-1 rounded-lg" style={{ background: "#f5f3ff", color: "#7c3aed" }}
                                                    onClick={() => handleStatusChange(o.poId, "Billed")}>Bill</button>
                                            )}
                                            {o.billId && <span className="text-xs ml-2" style={{ color: "var(--black-400)" }}>{o.billId}</span>}
                                        </td>
                                    </tr>
                                );
                            })}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.4)" }}>
                    <div className="bg-white rounded-2xl p-8 shadow-2xl overflow-y-auto" style={{ width: 640, maxHeight: "85vh", border: "1px solid var(--card-border)" }}>
                        <div className="flex items-center justify-between mb-6">
                            <h2 className="text-lg font-bold" style={{ color: "var(--foreground)" }}>New Purchase Order</h2>
                            <button onClick={() => setShowModal(false)} className="p-1.5 rounded-lg hover:bg-gray-100"><X size={16} /></button>
                        </div>
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--black-500)" }}>Vendor *</label>
                                ...
                                <div className="flex items-center gap-3 pt-5">
                                    <input type="checkbox" id="po-interstate" checked={form.isInterState} onChange={e => setForm({ ...form, isInterState: e.target.checked })} />
                                    <label htmlFor="po-interstate" className="text-xs font-semibold" style={{ color: "var(--black-500)" }}>Inter-State (IGST)</label>
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-semibold mb-1 block" style={{ color: "var(--black-500)" }}>Expected Delivery</label>
                                <input type="date" className="w-full px-3 py-2 text-sm rounded-xl border" style={{ borderColor: "var(--card-border)" }}
                                    value={form.expectedDelivery} onChange={e => setForm({ ...form, expectedDelivery: e.target.value })} />
                            </div>
                            <div>
                                ...
                                {form.items.map((item, i) => (
                                    <div key={i} className="grid grid-cols-12 gap-2 mb-2 items-center">
                                        ...
                                        <div className="col-span-1 text-right text-xs font-bold">{formatINR(Number(item.quantity) * Number(item.rate))}</div>
                                        <button className="col-span-1 p-1 rounded hover:bg-red-50" onClick={() => removeItem(i)}><X size={12} style={{ color: "#ef4444" }} /></button>
                                    </div>
                                ))}
                                <button className="text-xs font-semibold px-3 py-1.5 rounded-lg" style={{ background: "var(--black-50)", color: "var(--teal-600)" }} onClick={addItem}>+ Add Item</button>
                            </div>
                        </div>
