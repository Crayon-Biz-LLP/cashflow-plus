...
async function seedDatabase() {
    ...
    // Seed Tax Profiles (Global Slabs)
    const taxCount = await TaxProfile.countDocuments();
    if (taxCount === 0) {
        await TaxProfile.insertMany([
            // India GST Slabs
            { profileId: "TAX-IN-001", name: "GST 0% (Exempt)", taxType: "GST", rate: 0, cgstRate: 0, sgstRate: 0, igstRate: 0, isDefault: false, description: "Exempted goods & services" },
            { profileId: "TAX-IN-002", name: "GST 5%", taxType: "GST", rate: 5, cgstRate: 2.5, sgstRate: 2.5, igstRate: 5, isDefault: false, description: "Essential items" },
            { profileId: "TAX-IN-003", name: "GST 12%", taxType: "GST", rate: 12, cgstRate: 6, sgstRate: 6, igstRate: 12, isDefault: false, description: "Standard rate tier 1" },
            { profileId: "TAX-IN-004", name: "GST 18%", taxType: "GST", rate: 18, cgstRate: 9, sgstRate: 9, igstRate: 18, isDefault: true, description: "Standard rate — most services" },
            { profileId: "TAX-IN-005", name: "GST 28%", taxType: "GST", rate: 28, cgstRate: 14, sgstRate: 14, igstRate: 28, isDefault: false, description: "Luxury & sin goods" },

            // USA Sales Tax Slabs
            { profileId: "TAX-US-NY", name: "US Sales Tax - NY (8.875%)", taxType: "Sales Tax", rate: 8.875, isDefault: false, description: "New York State & City Sales Tax" },
            { profileId: "TAX-US-CA", name: "US Sales Tax - CA (7.25%)", taxType: "Sales Tax", rate: 7.25, isDefault: false, description: "California State Sales Tax" },
            { profileId: "TAX-US-TX", name: "US Sales Tax - TX (6.25%)", taxType: "Sales Tax", rate: 6.25, isDefault: false, description: "Texas State Sales Tax" },
            { profileId: "TAX-US-FL", name: "US Sales Tax - FL (6.00%)", taxType: "Sales Tax", rate: 6.0, isDefault: false, description: "Florida State Sales Tax" },

            // TDS/Withholding
            { profileId: "TAX-IN-TDS-10", name: "TDS 10% (Professional)", taxType: "TDS", rate: 10, cgstRate: 0, sgstRate: 0, igstRate: 0, isDefault: false, description: "TDS u/s 194J — Professional services" },
            { profileId: "TAX-IN-TDS-2", name: "TDS 2% (Contractor)", taxType: "TDS", rate: 2, cgstRate: 0, sgstRate: 0, igstRate: 0, isDefault: false, description: "TDS u/s 194C — Contractor payments" },
        ]);
        console.log("   💰 Tax Profiles seeded (GST + USA Sales Tax + TDS)");
    }
...
app.get("/api/tax-profiles", authMiddleware, async (req, res) => {
    try {
        const allProfiles = await TaxProfile.find({ isActive: true }).sort({ name: 1 });
...
app.post("/api/invoices", authMiddleware, async (req, res) => {
    try {
        const { amount, taxProfileId } = req.body;
        const includeTax = req.body.includeGst !== false; // Backwards compatible with existing toggle
        ...
        if (includeTax) {
            if (taxProfileId) {
                const profile = await TaxProfile.findOne({ profileId: taxProfileId });
                if (profile) {
                ...
                // Default fallback (Legacy behaviour)
                taxRate = 18;
                taxType = "GST";
        ...
        const newInvoice = await Invoice.create({
            ...
            ...req.body,
            gst: taxAmount, // Keeping legacy field for old UI compatibility
            taxAmount,
            taxRate,
            taxType,
...
app.post("/api/expenses", authMiddleware, async (req, res) => {
    try {
        const { caseId, amount, category, description, taxProfileId } = req.body;
        ...
        const newExpense = await Expense.create({
            ...
            amount,
            gstAmount: taxAmount, // Legacy compatibility
            taxAmount,
            taxRate,
            taxType,
            totalWithGst: amount + taxAmount,
...
app.get("/api/dashboard/stats", authMiddleware, async (req, res) => {
    try {
        ...
        // Timeline Alerts for Notification Mapping
        const invoicesForAlerts = await Invoice.find({ status: { $ne: "Paid" } });
        const billsForAlerts = await Bill.find({ status: { $ne: "Paid" } });
        const timelineAlerts = [
            ...getTimelineAlerts(invoicesForAlerts, "Invoice"),
            ...getTimelineAlerts(billsForAlerts, "Bill")
        ];
